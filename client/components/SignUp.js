    ...
    return (
        <div className="w-full h-full flex flex-col items-center justify-center">
            <div className="text-center grid grid-rows-3 gap-2 gradient p-8 rounded-md">
                ...
                <input type="text" className="text-2xl text-center rounded-md p-2 my-2 text-blue-400 placeholder-blue-300 focus:outline-none"
                value={input}
                onChange={(e) => {setInput(e.target.value)}}
                onKeyDown={(e) => {e.key === "Enter" && addUser()}}
                />
                <button 
                className={`w-full text-xl text-white py-2 px-3 font-bold rounded-md ${input ? "bg-sky-400" : "bg-slate-400"}`}
                disabled={!input}
                onClick={addUser}
                >
                    Join Chat
